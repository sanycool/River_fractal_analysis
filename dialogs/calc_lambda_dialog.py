from PyQt5.QtWidgets import (
    QDialog, QLabel, QPushButton, QHBoxLayout, QVBoxLayout, QMessageBox,
    QFileDialog, QScrollArea, QWidget, QLineEdit, QSpinBox, QTextBrowser)
...
from data_processing.lambda_calculator import LambdaCalculatorWorker
...
class Ui_DialogCalcLambda:
    ...
    def setupUi(self, window):
        ...
        self._setup_settings_layout(window)

    def _setup_my_window(self, window):
        """Установка заголовка и размера окна"""
        window.setWindowTitle("Рассчитать λ (лямбда)")
    ...
    def _setup_right_layout(self, parent):
        ...
        # --- Создание области выбора колонок параметров обработки ---
        self.settings_layout = QVBoxLayout(parent)
        self.right_layout.addLayout(self.settings_layout)
    ...
    def _setup_settings_layout(self, Dialog):
        """Настройка layout'а виджетов для параметров обработки"""
        self.settings_label = QLabel("Настройки гистограммы")
        self.settings_label.setFont(FONT_10)
        self.settings_layout.addWidget(self.settings_label, alignment=Qt.AlignCenter)

        self.interval_step_layout = QHBoxLayout()
        self.settings_layout.addLayout(self.interval_step_layout)

        self.interval_step_label = QLabel("Шаг интервала гистограммы:")
        self.interval_step_layout.addWidget(self.interval_step_label)
        self.interval_step_value = QLineEdit()
        self.interval_step_value.setValidator(QIntValidator(1, int(1e9)))
        self.interval_step_layout.addWidget(self.interval_step_value)
        self.interval_step_layout.addStretch()

        self.min_approximation_label = QLabel("минимальный интервал аппроксимации:")
        self.interval_step_layout.addWidget(self.min_approximation_label)

        self.min_approximation_interval_spinbox = QSpinBox(Dialog)
        self.min_approximation_interval_spinbox.setMinimum(4)
        self.min_approximation_interval_spinbox.setFixedWidth(50)
        self.interval_step_layout.addWidget(self.min_approximation_interval_spinbox)

        length_limits_layout, self.lower_length_limit_value, self.upper_length_limit_value = limits_fields_maker(
            label_text="Порог длины русла:")
        self.settings_layout.addLayout(length_limits_layout)

class DialogCalcLambda(QDialog, Ui_DialogCalcLambda):
    def __init__(self, data: pd.DataFrame, parent=None):
        ...
        self.grid_code_column: str | None = None           # Колонка с порядками водотоков
        self.river_id_column: str | None = None            # Колонка с ID водотоков
        self.river_length_column: str | None = None        # Колонка с длинами водотоков
        self.active_ranges: dict | None = None             # Словарь диапазонов (лимиты) для выбранных порядков
        self.interval_step: int | None = None              # Значение шага интервала гистограммы
        self.length_limits: list | None = None             # Ограничения [min, max] длины всех водотоков
        self.min_approximate_interval: int | None = None   # Значение минимального интервала аппроксимации

        self.selected_grid_codes = []   # Список для хранения выбранных порядков
        self.gridcode_ranges = {}       # Словарь для хранения диапазонов {gridcode: (min_lineedit, max_lineedit)}
        ...
        apply_button_style([self.cancel_button, self.calculate_button, self.done_button,
                                 self.perimeter_button, self.area_button, self.save_button,
                                 self.show_length_distribution_button, self.help_button])
        apply_separator_style([self.separator1, self.separator2, self.separator3])

        # --- Связываем кнопки и сигналы изменения выбора с действиями ---
    ...
    def start_calculation(self):
        ...
        if (not self._collect_main_bass_parameters()
                or not self._collect_rivers_parameters() or not self._collect_histogram_parameters()):
    ...
    def _collect_main_bass_parameters(self):
        ...
        try:
            ...
            self.main_perimeter_limits = get_limits_values(
                self.min_main_perimeter.text(),
                self.max_main_perimeter.text())
            self.main_area_limits = get_limits_values(
    ...
    def _collect_rivers_parameters(self):
        """Собирает параметры рек и проверяет наличие выбранных порядков в selected_grid_codes. Возвращает True при успехе"""
        try:
            # Параметры русел
            ...
            QMessageBox.warning(self, "Ошибка обработки параметров рек", str(e))
            return False

    def _collect_histogram_parameters(self):
        """Собирает параметры гистограммы. Возвращает True при успехе"""
        try:
            # Проверка interval_step
            try:
                self.interval_step = int(self.interval_step_value.text())
            except ValueError:
                QMessageBox.warning(self, "Ошибка", "Не веррно указан шаг интервала гистограммы!")
                return False

            # Получение интервала длины русел
            self.length_limits = get_limits_values(
                self.lower_length_limit_value.text(),
                self.upper_length_limit_value.text())
            self.length_limits[0] = self.parameters_statistics["parameters_min"]\
                if self.length_limits[0] is None else self.length_limits[0]
            self.length_limits[1] = self.parameters_statistics["parameters_max"]\
                if self.length_limits[1] is None else self.length_limits[1]

            # Получение минимального интервала аппроксимации
            self.min_approximate_interval = self.min_approximation_interval_spinbox.value()

            return True
        except Exception as e:
            QMessageBox.warning(self, "Ошибка обработки параметров гистограммы", str(e))
            return False
    ...
    def _setup_worker_and_thread(self):
        ...
        self.worker = LambdaCalculatorWorker()
        ...
        self.worker.set_bass_area_limits(self.main_area_limits)
        self.worker.set_bass_perimeter_limits(self.main_perimeter_limits)
        ...
        self.worker.set_interval_step(self.interval_step)
        self.worker.set_length_limits(self.length_limits)
        self.worker.set_min_approximate_interval(self.min_approximate_interval)
        ...
        self.thread.started.connect(lambda: self.worker.calculate_lambda(self.data))
    ...
    def _update_progress(self, value: int, log_message: str):
